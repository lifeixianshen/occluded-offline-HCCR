...
def save_all_weights(d, g, epoch_number, current_loss):
    ...
    save_dir = os.path.join(BASE_DIR, f'{now.month}{now.day}')
    if not os.path.exists(save_dir):
        os.makedirs(save_dir)
    g.save_weights(
        os.path.join(save_dir, f'generator_{epoch_number}_{current_loss}.h5'),
        True,
    )
    d.save_weights(
        os.path.join(save_dir, f'discriminator_{epoch_number}.h5'), True
    )
...
def train_multiple_outputs(n_images, batch_size, epoch_num, critic_updates=5):
    ...
    for epoch in range(epoch_num):
        ...
        print(f'batch {epoch} d_loss : {np.mean(d_losses)}')
        ...
        d_on_g_losses = [d_on_g_loss]
        print(f'batch {epoch} d_on_g_loss : {d_on_g_loss}')
        ...
        if epoch % 100 == 0:
            ...
            for i in range(3):
                ...
                cv2.imwrite(
                    f'/home/alyssa/PythonProjects/occluded/key_code/img_inpainting/out/{str(epoch)}_{str(i)}.jpg',
                    output,
                )
